package com.river.behaviorPlugin.util
...
object TransformUtil {
     ...
     * @param outputProvider TransformOutputProvider
     */
    private fun handleJarInputInsert(
        jarInput: JarInput,
        outputProvider: TransformOutputProvider,
        callback: HandleByteCode
    ) {
    ...
    fun handleDirInput(
        ...
        if (incremental) {
            if (callback.onlyMap()) {
                ...
                directoryInput.changedFiles.forEach { changeFile ->
                    val destFile =
                        File(changeFile.key.absolutePath.replace(srcDirPath, destDirPath))
                    when (changeFile.value) {
                        Status.NOTCHANGED -> {
                        }
                        Status.REMOVED -> {
    ...
    private fun filterFile(fileName: String): Boolean {
        return fileName.endsWith(".class") &&
                ...
                !fileName.startsWith("com/chad/library/adapter") &&
                fileName != "R.class" &&
                fileName != "R2.class" &&
                fileName != "BuildConfig.class" ||
                fileName == "androidx/appcompat/app/AppCompatDelegateImpl.class" ||
                fileName == "androidx/fragment/app/Fragment.class"
    }
